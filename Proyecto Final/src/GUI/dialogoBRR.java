...
public class dialogoBRR extends JPanel {
    ...
    private int dialogoActualEspecial = 1;
    private final int TOTAL_DIALOGOS = 5;

    private final int TOTAL_DIALOGOS_ESPECIALES = 4;
    ...
    private void cargarImagenDialogo() {
        ...
        if (EstadoJuego.isPuedeMostrarDialogosEspeciales()) {
            ruta = "src/resources/images/dialogoBRR_especial" + dialogoActualEspecial + ".png";
        } else {
            // Usar diálogos normales
            ruta = "src/resources/images/dialogoBRR" + dialogoActual + ".png";
        }
        
        // Si se está mostrando el último diálogo especial, guardar el tiempo de juego
        try {
            if (EstadoJuego.isPuedeMostrarDialogosEspeciales() &&
                dialogoActualEspecial == TOTAL_DIALOGOS_ESPECIALES &&
                !EstadoJuego.isBrrHablado()) {
                // Guardar el record (milisegundos) y marcar como hablado para no repetir
                long elapsed = EstadoJuego.getElapsedMillis();
                RankingManager.saveRecordIfApplicable(elapsed);
                EstadoJuego.setBrrHablado(true);
            }
        } catch (Exception ex) {
            System.err.println("Error guardando ranking BRR: " + ex.getMessage());
        }
    ...
    private void avanzarSiguiente() {
        if (EstadoJuego.isPuedeMostrarDialogosEspeciales()) {
            dialogoActualEspecial++;
            if (dialogoActualEspecial <= TOTAL_DIALOGOS_ESPECIALES) {
                cargarImagenDialogo();
                iniciarFadeIn();
            } else {
                terminarDialogo();
    ...
    private void terminarDialogo() {
        ...
        if (parentFrame != null) {
            if (EstadoJuego.isPuedeMostrarDialogosEspeciales()) {
                ...
                parentFrame.getContentPane().add(inicioPanel);
    ...
    private void dibujarIndicadorContinuar(Graphics2D g2d, int panelWidth, int panelHeight) {
        ...
        if (EstadoJuego.isPuedeMostrarDialogosEspeciales()) {
            if (dialogoActualEspecial < TOTAL_DIALOGOS_ESPECIALES) {
                texto = "Haz CLICK para continuar";
            } else {
    ...
    private void dibujarContadorProgreso(Graphics2D g2d, int panelWidth) {
        ...
        String progreso;
        if (EstadoJuego.isPuedeMostrarDialogosEspeciales()) {
            progreso = dialogoActualEspecial + "/" + TOTAL_DIALOGOS_ESPECIALES;
        } else {
            progreso = dialogoActual + "/" + TOTAL_DIALOGOS;
        }
